...
function startGame() {
    ...
    obstacle_timer = setInterval(createObstacles, 5000);
}
...
function move() {
    ...
    if (!isSnakeUnit(new_unit) && new_unit !== undefined && !obstacleOnPath(new_unit)) {
...
function createObstacles() {
    ...
    while (!obstacle){
        ...
        var obstacleCell = document.getElementsByClassName("cell-" + obstacleY + "-" + obstacleX)[0];
        var obstacleCellClasses = obstacleCell.getAttribute("class").split(" ");
        if(!obstacleCellClasses.includes("snake-unit")){
            obstacleCell.classList.add('problem-unit');
            obstacle = true;
        }
    }
}
